...
- remove the measured Rust-path latency regressions (median +62.9% to +177.8% across the four workloads) and re-run the same apples-to-apples performance protocol;
...
`pilot.toml` is the machine-readable status record. A pending or failed item remains explicit rather than being inferred from a successful narrower test. The first valid Linux performance run completed all 120 paired invocations with stable CPU policy and recorded only a 67,584-byte steady-state RSS regression, but latency failed the configured budget substantially: median regressions were +122.7%, +177.8%, +62.9%, and +119.8% for the four workloads. The raw samples are retained under `evidence/` and the failure remains part of the M3 record until a changed implementation is remeasured. `exposure-sources.toml` records the exact active production source inventory used by `chromifer measure-migration-exposure`. On the current combined candidate, the reproducible measurement reduces authored memory-unsafe LOC from 98 to 97, active implementation files from 4 to 3, and manual raw-pointer fields from 2 to 1. The candidate has two authored branch points versus one in the baseline, and authored production LOC grows from 98 to 160 because of the Rust/CXX boundary, so the existing maintenance gate is also intentionally not marked passed.
