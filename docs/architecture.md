...
Candidate ranking retains the module/required-target compatibility matrix as declaration coverage. It can also consume a deterministic LLVM source-coverage report bound to the exact manifest digest and source inventory. When measured coverage is present, source-measurement completeness and covered-line ratio replace the declaration proxy as the coverage-risk term; missing manifest sources remain an explicit blocking concern. Actual gate execution is recorded separately as content-addressed evidence: the exact manifest digest, execution definitions, hashed inputs, results, timings, and full output logs can be re-verified before a transition check consumes them. Optional checkout attestation binds the evidence runner's Git worktree and direct executables. Detached Ed25519 attestations can bind the exact evidence bytes to an externally pinned runner public key without changing the evidence digest. A separate Chromium checkout lock binds a nested gclient workspace to an exact source revision, DEPS/gclient metadata, GN args, Ninja output, normalized project export, and required target semantics. Production runner isolation, signing-key custody, and transparency-backed source/workload identity remain external trust requirements.
...
The same harness can use Chromium's existing `rust_static_library.gni`. A pinned gclient workspace records the source and depot_tools revisions, native template, bundled Rustc, GN, Ninja, deterministic args, generated graph, required target semantics, and endpoint result. The native report stores source-relative tool paths and content identities rather than host paths, while the checkout lock separately binds the exact project and Ninja outputs.
