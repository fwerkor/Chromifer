...
The active pilot is the `services/metrics` UKM factory/recorder Mojo path at pinned Chromium revision `008cdad85f0721c89b42ef4dcaabcee615482609`. Rust owns `UkmRecorderFactory` dispatch together with its `UkmRecorderInterface` child receivers while the existing C++ factory entry point and Mojom contract remain stable. The Linux focused contract path, source-separated C++ rollback path, and `//components/ukm:ukm_unittests` regression suite are verified. Reproducible exposure measurement shows a strict reduction in authored memory-unsafe LOC and raw-pointer fields, but the maintenance gate remains open because total authored production LOC grows at the Rust/CXX boundary. A valid 15-sample Linux performance comparison also exposed substantial latency regressions in the current boundary design (median +62.9% to +177.8% across the four configured workloads), while its RSS budget passed. The pilot remains in progress until the boundary is optimized, the performance gate is remeasured successfully, and the supported-desktop and maintenance criteria pass.
