...
const CONSUMER_CONTRACT_SCHEMA_VERSION: u32 = 1;
...
pub struct GenerateOptions {
    ...
    pub additional_consumers: Vec<ConsumerOptions>,
    pub consumer_contract: Option<PathBuf>,
    pub force: bool,
    pub check: bool,
}
...
pub struct ConsumerContract {
    ...
    pub consumers: Vec<ConsumerOptions>,
}
...
pub struct BridgeProvenance {
    ...
    pub additional_consumers: Vec<ConsumerProvenance>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub consumer_contract: Option<ConsumerContractProvenance>,
    pub build_gn_sha256: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ConsumerContractProvenance {
    pub path: String,
    pub sha256: String,
}
...
pub enum BuildBridgeError {
    ...
    #[error("failed to parse C++ consumer contract JSON: {0}")]
    ParseConsumerContract(serde_json::Error),
    #[error(
        "unsupported C++ consumer contract schema version {found}; supported version is {supported}"
    )]
    UnsupportedConsumerContractSchema { found: u32, supported: u32 },
    #[error("C++ consumer contract must declare at least one consumer")]
    EmptyConsumerContract,
    #[error("C++ consumer contract `{0}` must be a regular file inside the selected package")]
    InvalidConsumerContractPath(String),
    #[error("C++ consumer options do not match consumer contract `{0}`")]
    ConsumerContractMismatch(String),
    #[error("additional C++ consumers require a package-local consumer contract")]
    AdditionalConsumersRequireContract,
    ...
    #[error("C++ consumer target `{0}` is declared more than once")]
    DuplicateConsumerTarget(String),
    #[error("C++ consumer selects CXX bridge `{0}` which is not generated by this Rust target")]
    UnknownConsumerCxxBinding(String),
...
struct RenderInputs<'a> {
    ...
    additional_consumers: &'a [ConsumerProvenance],
}

pub fn parse_consumer_contract(bytes: &[u8]) -> Result<Vec<ConsumerOptions>, BuildBridgeError> {
    let contract: ConsumerContract =
        serde_json::from_slice(bytes).map_err(BuildBridgeError::ParseConsumerContract)?;
    if contract.schema_version != CONSUMER_CONTRACT_SCHEMA_VERSION {
        return Err(BuildBridgeError::UnsupportedConsumerContractSchema {
            found: contract.schema_version,
            supported: CONSUMER_CONTRACT_SCHEMA_VERSION,
        });
    }
    if contract.consumers.is_empty() {
        return Err(BuildBridgeError::EmptyConsumerContract);
    }
    Ok(contract.consumers)
}

fn prepare_consumer_contract(
    package_root: &Path,
    path: Option<&Path>,
    primary: Option<&ConsumerOptions>,
    additional: &[ConsumerOptions],
) -> Result<Option<ConsumerContractProvenance>, BuildBridgeError> {
    let Some(path) = path else {
        if additional.is_empty() {
            return Ok(None);
        }
        return Err(BuildBridgeError::AdditionalConsumersRequireContract);
    };
    let metadata = fs::symlink_metadata(path)
        .map_err(|_| BuildBridgeError::InvalidConsumerContractPath(display_path(path)))?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(BuildBridgeError::InvalidConsumerContractPath(display_path(
            path,
        )));
    }
    let canonical = path
    ...
    let relative = normalize_repo_relative_path(&relative.to_string_lossy())
        .ok_or_else(|| BuildBridgeError::InvalidConsumerContractPath(display_path(path)))?;
    let bytes = read_file(&canonical)?;
    let parsed = parse_consumer_contract(&bytes)?;
    let expected: Vec<_> = primary
        ...
        .collect();
    if parsed != expected {
        return Err(BuildBridgeError::ConsumerContractMismatch(relative));
    }
    Ok(Some(ConsumerContractProvenance {
        path: relative,
...
pub fn generate_bridge(options: &GenerateOptions) -> Result<GeneratedBridge, BuildBridgeError> {
    ...
    let generated_cxx_headers = generated_cxx_headers(
        ...
        &cxx_bindings,
        options.consumer.is_some() || !options.additional_consumers.is_empty(),
    )?;
    ...
    for options in &options.additional_consumers {
        if let Some(consumer) = prepare_consumer(
            &selected.root,
            &target_name,
            Some(options),
            gn_package_path.as_deref(),
            ...
            additional_consumers.push(consumer);
        }
    }
    validate_consumer_targets(&target_name, consumer.as_ref(), &additional_consumers)?;
    let consumer_contract = prepare_consumer_contract(
        &selected.root,
        options.consumer_contract.as_deref(),
        options.consumer.as_ref(),
        &options.additional_consumers,
    )?;
    ...
    let provenance = BridgeProvenance {
        ...
        additional_consumers: additional_consumers.clone(),
        consumer_contract,
    ...
    let summary = GenerateSummary {
        ...
        consumer_count: usize::from(consumer.is_some()) + additional_consumers.len(),
...
fn prepare_consumer(
    ...
    let selected_cxx_bindings = if options.cxx_bindings.is_empty() {
        cxx_bindings.to_vec()
    } else {
        let known: BTreeSet<_> = cxx_bindings.iter().cloned().collect();
        let mut selected = BTreeSet::new();
        for binding in &options.cxx_bindings {
            let relative = normalize_repo_relative_path(binding)
                .ok_or_else(|| BuildBridgeError::InvalidConsumerSource(binding.clone()))?;
            if !known.contains(&relative) {
                return Err(BuildBridgeError::UnknownConsumerCxxBinding(relative));
            }
            selected.insert(relative);
        }
        selected.into_iter().collect()
    };
    let generated_header_by_binding: BTreeMap<_, _> = cxx_bindings
        .iter()
        .cloned()
        .zip(generated_headers.iter().cloned())
        .collect();
    let mut required_headers = BTreeSet::new();
    for binding in &selected_cxx_bindings {
        if let Some(header) = generated_header_by_binding.get(binding) {
            required_headers.insert(header.clone());
        }
    }
    ...
    Ok(Some(ConsumerProvenance {
        ...
        sources,
        cxx_bindings: selected_cxx_bindings,
        required_headers,
        deps,
        public_deps,
        visibility,
...
fn validate_consumer_targets(
    rust_target_name: &str,
    primary: Option<&ConsumerProvenance>,
    additional: &[ConsumerProvenance],
) -> Result<(), BuildBridgeError> {
    let mut targets = BTreeSet::from([rust_target_name.to_owned()]);
    for consumer in primary.into_iter().chain(additional) {
        if !targets.insert(consumer.target_name.clone()) {
            return Err(BuildBridgeError::DuplicateConsumerTarget(
...
fn render_build_gn(inputs: &RenderInputs<'_>) -> String {
    ...
    if let Some(consumer) = inputs.consumer {
        output.push('\n');
        render_consumer(&mut output, consumer);
    }
    for consumer in inputs.additional_consumers {
        output.push('\n');
        render_consumer(&mut output, consumer);
    }
    output
...
fn render_consumer(output: &mut String, consumer: &ConsumerProvenance) {
    output.push_str(&format!(
        "source_set(\"{}\") {{\n",
    ...
    if !consumer.deps.is_empty() {
        render_list(output, "deps", consumer.deps.iter().map(String::as_str));
    }
...
mod tests {
    ...
    #[test]
    fn generates_multiple_consumers_with_distinct_cxx_bridge_subsets() {
        let tree = TempTree::new();
        tree.write(
            "cxx/Cargo.toml",
            "[package]\nname='cxx'\nversion='1.0.0'\nedition='2021'\n[lib]\npath='lib.rs'\n",
        ...
        tree.write("src/alpha.rs", "#[cxx::bridge]\nmod ffi {}\n");
        tree.write("src/beta.rs", "#[cxx::bridge]\nmod ffi {}\n");
        tree.write(
            "consumer/alpha.cc",
            "#include \"services/example/rust/src/alpha.rs.h\"\nint Alpha() { return 1; }\n",
        );
        tree.write(
            "consumer/beta.cc",
            "#include \"services/example/rust/src/beta.rs.h\"\nint Beta() { return 2; }\n",
        );
        let contract = r#"{
  "schema_version": 1,
  "consumers": [
...
"#;
        tree.write("consumers.json", contract);
        let consumers = parse_consumer_contract(contract.as_bytes()).unwrap();
        let mut options = options(&tree);
        options.gn_package_path = Some("//services/example/rust".into());
        options.consumer = Some(consumers[0].clone());
        options.additional_consumers = consumers[1..].to_vec();
        options.consumer_contract = Some(tree.root.join("consumers.json"));

        let generated = generate_bridge(&options).unwrap();
        assert_eq!(generated.summary.consumer_count, 2);
        assert!(generated.build_gn.contains("source_set(\"alpha_cpp\")"));
        assert!(generated.build_gn.contains("source_set(\"beta_cpp\")"));
        let provenance: BridgeProvenance =
            serde_json::from_str(&generated.provenance_json).unwrap();
        assert_eq!(
            provenance.consumer.as_ref().unwrap().cxx_bindings,
            vec!["src/alpha.rs"]
        );
        assert_eq!(
            provenance.additional_consumers[0].cxx_bindings,
            vec!["src/beta.rs"]
        );
        let contract_provenance = provenance.consumer_contract.unwrap();
        assert_eq!(contract_provenance.path, "consumers.json");
        assert_eq!(contract_provenance.sha256, sha256_hex(contract.as_bytes()));

        let mut unknown = options.clone();
        unknown.additional_consumers[0].cxx_bindings = vec!["src/missing.rs".into()];
        unknown.consumer_contract = None;
        assert!(matches!(
            generate_bridge(&unknown),
            Err(BuildBridgeError::UnknownConsumerCxxBinding(binding)) if binding == "src/missing.rs"
        ));

        let mut duplicate = options;
        duplicate.additional_consumers[0].target_name = "alpha_cpp".into();
        duplicate.consumer_contract = None;
        assert!(matches!(
            generate_bridge(&duplicate),
            Err(BuildBridgeError::DuplicateConsumerTarget(target)) if target == "alpha_cpp"
        ));
    }
