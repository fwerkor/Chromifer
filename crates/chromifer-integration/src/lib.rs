...
fn append_integration_targets(
    ...
    let block = integration_targets(contract, &consumer.target_name);
...
fn integration_targets(contract: &IntegrationContract, consumer_target: &str) -> String {
    format!(
...
mod tests {
    ...
    #[test]
    fn endpoint_target_propagates_repository_root_includes() {
        ...
        let targets = integration_targets(&contract, "c_abi_smoke");
        assert!(targets.contains("include_dirs = [ \"//\" ]"));
        assert!(targets.contains("deps = [ \":c_abi_smoke\" ]"));
        assert!(targets.contains("group(\"integration\")"));
    }
