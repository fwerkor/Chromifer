...
use chromifer_components::{
    AnalysisOptions, CandidateConcern, ComponentAnalysis, analyze_components_with_coverage,
};
use chromifer_coverage::{
    CoverageOptions, generate_and_write as generate_coverage, load_report as load_coverage_report,
...
enum Command {
    ...
    RankComponents {
        ...
        manifest: PathBuf,
        /// Optional measured source coverage report generated by summarize-coverage.
        #[arg(long)]
    ...
    /// Convert LLVM coverage export JSON into a deterministic source coverage report.
    SummarizeCoverage {
        /// Migration manifest whose source inventory is being measured.
        manifest: PathBuf,
        /// Source checkout root matching the manifest baseline.
        source_root: PathBuf,
        /// JSON emitted by `llvm-cov export`, typically with `--summary-only`.
        export: PathBuf,
        /// Deterministic Chromifer coverage report JSON.
        output: PathBuf,
        /// Replace an existing coverage report.
        #[arg(long, conflicts_with = "check")]
        force: bool,
        /// Verify the committed report without modifying it.
        #[arg(long, conflicts_with = "force")]
        check: bool,
...
fn run(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    match cli.command {
        ...
        Command::RankComponents {
            ...
            let manifest = Manifest::load(&manifest)?;
            let coverage = coverage.as_deref().map(load_coverage_report).transpose()?;
            if let Some(coverage) = &coverage {
                verify_report_manifest(coverage, &manifest, &manifest_bytes)?;
            }
            let analysis = analyze_components_with_coverage(
                &manifest,
                &AnalysisOptions { path_depth },
                coverage.as_ref(),
            )?;
            if json {
        ...
        Command::SummarizeCoverage {
            ...
            let summary = generate_coverage(&CoverageOptions {
                manifest,
                source_root,
                export,
                output,
            ...
            if json {
                ...
                println!(
                    "current: {} measured source(s) across {} module(s), {}/{} covered lines ({:.2}%)",
                    summary.measured_sources,
                    summary.modules,
                    summary.covered_lines,
                    summary.total_lines,
                    f64::from(summary.line_basis_points) / 100.0
                );
            } else {
                println!(
                    "measured {} of {} manifest source(s) across {} module(s); missing {}; line coverage {}/{} ({:.2}%)",
                    summary.measured_sources,
                    summary.manifest_sources,
                    summary.modules,
                ...
                println!("wrote coverage report to {}", summary.output);
            }
        }
        ...
        Command::ValidateMigration { directory } => {
            let evidence = MigrationEvidence::load(&directory)?;
            println!(
                "valid migration {}: status={:?}, revision={}, implementation={:?}, parity={:?}, performance={:?}, exposure={:?}, memory_safety={:?}, maintenance={:?}, rollback={:?}",
                evidence.pilot.id,
...
fn print_component_ranking(analysis: &ComponentAnalysis, limit: usize) {
    ...
    for candidate in analysis.ranking.iter().take(limit) {
        ...
        if let Some(coverage) = analysis
            .components
            .iter()
            .find(|component| component.id == candidate.component)
            .and_then(|component| component.source_coverage.as_ref())
        {
            println!(
                "   coverage: {}/{} sources measured, {}/{} lines ({:.2}%), effective {:.2}%",
                coverage.measured_sources,
                coverage.total_sources,
                coverage.covered_lines,
                coverage.total_lines,
                f64::from(coverage.line_basis_points) / 100.0,
                f64::from(coverage.effective_basis_points) / 100.0
            );
        }
...
fn display_concern(concern: &CandidateConcern) -> String {
    match concern {
        ...
        CandidateConcern::IncompleteSourceCoverage {
            ...
            "coverage report misses {missing_sources} of {total_sources} component source files"
        ),
